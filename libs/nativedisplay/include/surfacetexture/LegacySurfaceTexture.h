...
#include "LegacyEGLConsumer.h"
#include "LegacyImageConsumer.h"
 ...
 */
class ANDROID_API LegacySurfaceTexture : public ConsumerBase {
public:
    /**
     ...
     */
    typedef status_t (*LegacySurfaceTexture_fenceWait)(int fence, void* passThroughHandle);
     ...
     */
    LegacySurfaceTexture(uint32_t tex, uint32_t textureTarget, bool useFenceSync,
                         bool isControlledByApp);

    LegacySurfaceTexture(uint32_t textureTarget, bool useFenceSync, bool isControlledByApp);

    LegacySurfaceTexture(const sp<IGraphicBufferConsumer>& bq, uint32_t tex, uint32_t textureTarget,
                         bool useFenceSync, bool isControlledByApp)
            __attribute((deprecated("Prefer ctors that create their own surface and consumer.")));

    LegacySurfaceTexture(const sp<IGraphicBufferConsumer>& bq, uint32_t textureTarget,
                         bool useFenceSync, bool isControlledByApp)
            __attribute((deprecated("Prefer ctors that create their own surface and consumer.")));
    ...
    /**
     * Interface for LegacySurfaceTexture callback(s).
     */
    struct LegacySurfaceTextureListener : public RefBase {
    ...
    /**
     * setSurfaceTextureListener registers a LegacySurfaceTextureListener.
     */
    void setSurfaceTextureListener(const sp<LegacySurfaceTextureListener>&);
...
protected:
    ...
    /**
     * The default consumer usage flags that LegacySurfaceTexture always sets on its
     * BufferQueue instance; these will be OR:d with any additional flags passed
     * from the LegacySurfaceTexture user. In particular, LegacySurfaceTexture will always
     ...
     */
    LegacyEGLConsumer mEGLConsumer;

    /**
     * mImageConsumer has LegacySurfaceTexture logic used when attached to a consumer
     * context (usually HWUI RenderThread).
     */
    LegacyImageConsumer mImageConsumer;

    /**
     * mLegacySurfaceTextureListener holds the registered LegacySurfaceTextureListener.
     * Note that LegacySurfaceTexture holds the lister with an sp<>, which means that the listener
     * must only hold a wp<> to LegacySurfaceTexture and not an sp<>.
     */
    sp<LegacySurfaceTextureListener> mLegacySurfaceTextureListener;

    friend class LegacyImageConsumer;
    friend class LegacyEGLConsumer;

private:
    void initialize();

    // Proxy listener to avoid having LegacySurfaceTexture directly implement FrameAvailableListener
    // as it is extending ConsumerBase which also implements FrameAvailableListener.
    class FrameAvailableListenerProxy : public ConsumerBase::FrameAvailableListener {
    public:
        FrameAvailableListenerProxy(const wp<LegacySurfaceTextureListener>& listener)
              : mLegacySurfaceTextureListener(listener) {}

    private:
        void onFrameAvailable(const BufferItem& item) override;

        const wp<LegacySurfaceTextureListener> mLegacySurfaceTextureListener;
    };
